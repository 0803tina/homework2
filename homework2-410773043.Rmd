...
```{r ans24}
allCommitDatesList <- as.character(unlist(allCommitDatesList))   
...
```{r ans25}
library(lubridate)
allCommitDatesList <- ymd_hms(allCommitDatesList)
datetime <- with_tz(allCommitDatesList, tz="Asia/Taipei")
datetime
```
...
```{r ans26}
library(stringr)
datetime1 <- hour(datetime)
datetime2 <- minute(datetime)
datetime3 <- datetime1*100 + datetime2
logicWithinTimeFrame <- datetime3 >= 430 & datetime3 <= 630
